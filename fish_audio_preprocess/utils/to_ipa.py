...
digit_dict = {'1': '一', '2': '二', '3': '三', '4': '四', '5': '五', '6': '六', '7': '七', '8': '八', '9': '九', '0': '零'}
...
def g2p(file_loc, label):
    ...
    try:
        ...
        logger.error(f'Error in {file_loc} with label {label} and new pinyin_label {new_pinyin_label}')
...
def chinese2p(input_file, pinyin_labels_file, phoneme_labels_file, num_workers=8):
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        with open(pinyin_labels_file, 'w', encoding='utf-8') as f_pinyin, open(phoneme_labels_file, 'w', encoding='utf-8') as f_phoneme:
            for task in tqdm(as_completed(tasks), total=len(tasks), desc='Converting to pinyin'):
                file_loc, phoneme, pinyin_label = task.result()
                if pinyin_label == '' or phoneme == '' or pinyin_label == None or phoneme == None or pinyin_label == ' ' or phoneme == ' ':
                    continue
